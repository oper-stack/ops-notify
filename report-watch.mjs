...
async function main() {
  ...
  try {
    ...
    let prospectsDone = 0;
    /**
     * Сколько всего времени у нас есть на этот прогон.
     *
     * Заданию в GitHub отведено сорок пять минут, а одна заявка может занять двадцать. Три
     * тяжёлых заявки подряд не влезают, и задание убили бы посередине: заявка осталась бы без
     * ярлыка, а покупатель без отчёта до следующего круга. Поэтому новую заявку берём только
     * если на неё осталось время. Очередь просыпается каждые пять минут, недобранное уедет
     * следующим прогоном, и это лучше, чем работа, убитая на середине.
     */
    const started = Date.now();
    const BUDGET_MS = Number(env('REPORT_RUN_BUDGET_MIN', '38')) * 60 * 1000;
    const NEEDED_MS = 21 * 60 * 1000;
    for (const item of TIDY ? [] : queue.slice(0, MAX_PER_RUN)) {
      if (Date.now() - started > BUDGET_MS - NEEDED_MS) {
        console.log('  время прогона на исходе: остальное берём следующим кругом через пять минут');
        break;
      }
